...
import { AddTests } from '../../components/Modals/Modals';
...
const Tests = () => {
    ...
    const fetchData = useCallback(async () => {
        setData([])
        await getApi(endPoints.getAllTests(pagination.page, pagination.limit, searchQuery), {
            setResponse: setData,
    ...
    const handleDelete = async (categoryId) => {
        ...
        await deleteApi(endPoints.deleteTests(categoryId), {
            setLoading,
                            ...
                            <tbody>
                                {loading ?
                                    ...
                                    Data?.tests?.length === 0 ?
                                        ...
                                        :
                                        Data?.tests?.map((i, index) => (
                                            <tr key={index}>
                                                <td>#{index + 1}</td>
                                                <td>{i?.title}</td>
                                                <td>{i?.mockTest?.name}</td>
                                                <td>{i?.totalQuestions}</td>
                                                <td>{i?.duration}</td>
                                                <td>{i?.totalMarks}</td>
                                                <td>{i?.isFree ? "Yes" : "No"}</td>
